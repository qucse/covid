...
class GCC {
  async getDataForCountry(country, toDate) {
    try {
      const response = await axios.get(
        "https://qu-covid19-api.herokuapp.com/data?country=" + country
      );
      ...
      let previousActive =
        previousLast.confirmed - (previousLast.recovered + previousLast.deaths);
      return {
        ...
        internationalTravelControls:
          lastData.international_movement_restrictions,
  ...
  async getDataForAllGCC(toDate) {
    let countriesResult = countries.map(
      async (country) => await this.getDataForCountry(country, toDate)
    );
