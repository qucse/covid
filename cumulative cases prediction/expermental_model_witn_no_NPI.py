...
from keras.layers import LSTM, Bidirectional, Dense, Input, Reshape
...
def forecast(model, cases, n_days, scaller):
    predictions = np.array([])
    for i in range(n_days):
        day_prediction = model.predict(cases)
...
callbacks = [ReduceLROnPlateau(monitor='val_loss', patience=4, verbose=1, factor=0.6),
             ...
             ModelCheckpoint(filepath='noNPI_project_model.h5', monitor='val_loss', save_best_only=True)]
...
for train, test in kfold.split(X, y):
    ...
    history = model.fit(cases_train, y[train],
                        epochs=300, verbose=2, batch_size=1, callbacks=callbacks, validation_data=(cases_test, y[test]))
    model.load_weights('noNPI_project_model.h5')
    # predicting new cases
    y_prediction = model.predict(cases_test, verbose=2)
    ...
    if mse <= np.min(loss_per_fold[:, 0]):
        model.save_weights('noNPI_best_fold_model.h5')
...
model.load_weights('noNPI_best_fold_model.h5')
...
true_data = cdata.query('id =="QAT" and date>="2020-08-01"')

days_to_predict = 8

inital_data = std_scaler.transform(
    true_data.iloc[0:n_steps_in][['confirmed_clean']].values.reshape(-1, 1).astype(float))
true_daily = np.diff(true_data.confirmed.iloc[n_steps_in:n_steps_in + days_to_predict].values)

forecast_data = forecast(model, np.expand_dims(inital_data.T, 1), int(days_to_predict / n_steps_out), std_scaler)
...
plt.xlabel('days')
plt.legend(['Prediction', 'Actual', 'smothed'], loc='best')
plt.show()
