...
cdata = cdata[cdata['date'] < "2020-09-01"]  # no cases before this date
...
for train, test in kfold.split(X, y):
    ...
    history = model.fit([cases_train, interventions_train.reshape(len(interventions_train), 1, n_steps_in * 5)],
                        y[train],
                        epochs=300, verbose=2, batch_size=1,
                        callbacks=callbacks,
                        validation_data=(
                            [cases_test, interventions_test.reshape(len(interventions_test), 1, n_steps_in * 5)],
                            y[test]))
...
model.load_weights('best_fold_model.h5')  # change this field to the weights file name if model training is False
...
true_data = cdata.query(
    'id =="QAT" and date>="2020-08-02"')  # keep one month of true data as initial data to the model.

days_to_predict = 16  # number of days to predict +1 , example if we are predicting 7 days , make this value as 8

inital_data = std_scaler.transform(
    true_data.iloc[0:n_steps_in][['confirmed_clean']].values.reshape(-1, 1).astype(float))

interventions = true_data.iloc[0:n_steps_in][
    ['school_closing', 'workplace_closing', 'gatherings_restrictions', 'transport_closing',
     'international_movement_restrictions']].values
npi = np.full((days_to_predict, 5), [1, 1, 1, 1, 1])  # here change npi to what ever the simulation scenario is.

forecast_data = forecast(model, np.expand_dims(inital_data.T, 1), np.append(interventions, npi, axis=0),
                         int(days_to_predict / n_steps_out),
                         std_scaler)
forecast_data = std_scaler.inverse_transform(forecast_data)
daily_cases_prediction = np.diff(np.insert(forecast_data, 0, true_data[0:n_steps_in].confirmed.iloc[-1]), axis=0)

plt.plot(daily_cases_prediction)
plt.title('what if prediction')
